...
description: "A liveblog documenting an early computer-vision project for tracking tennis matches."
date: 2022-09-16
categories: []
sports:
  - Tennis
...
Also, there's a community of tennis fans and analysts who are manually tracking games, based on a methodology [introduced by Jeff Sackman](https://www.tennisabstract.com/blog/2015/09/23/the-match-charting-project-quick-start-guide/). If the computer vision project is successful, it can hopefully help scale the tracking and we can get a more comprehensive account of tennis.

## [2022-09-17] Project Layout
...
### 2. Identify and map the court.

The ultimate goal is to locate the players and ball using a *common coordinate system*. The good news is professional tennis matches are played on regulation courts. If a few key aspects of the court are identified, then we can leverage the known court dimensions to create a common coordinate system.
...
### 5. Map the players and ball to the court.
...
## [2022-09-19] Classifying Game States 02
...
### Existing Court Detection Methods

**Tennis Court Detection**
Grzegorz Chlebus created a [repository](https://github.com/gchlebus/tennis-court-detection) on GitHub for his tennis court detector. The method was adapted from Farin D. et al. "Robust Camera Calibration for Sports Videos using Court Models". It was created 4 years ago and apparently requires a bit of [updating](https://github.com/gchlebus/tennis-court-detection/issues/8) in order to run, but unfortunately even with these updates I [couldn't get it to work](https://github.com/gchlebus/tennis-court-detection/issues/5). I can't confirm this, but apparently it can take up to 30 seconds to process one frame. This is hard to believe, but maybe the grid search just takes that long. I'm going to need something faster than this.

**Robust Camera Calibration for Sports Videos using Court Models**
This is the paper that Chlebus adapted in the first method on the list. The paper is available in full [here](https://www.researchgate.net/publication/220979520_Robust_camera_calibration_for_sport_videos_using_court_models). Notice the first word in the title is "robust", meaning the method was developed for various sports, some of which having dynamic camerawork where field vision is complex. This is a departure from the tennis broadcasts where camera perspectives barely change at all during gameplay. In the paper, they mention how a fitted model is necessary in their method because the court lines are not always obvious (model flow chart below). From tracking a few games, this just doesn't ring true for tennis so far. I'm going to be a little greedy here and try to get away with a set of heuristics based on the output from the hough line algo on openCV (memory, speed).

![Robust camera calibration model flow.](/figs/robust-camera-model-flow.png){fig-alt="Robust camera calibration model flow." width="60%"}

**Ben's Iterative Approach using RANSAC**
Ben, a user on Stack Overflow, suggested a possible solution for a [similar problem](https://stackoverflow.com/questions/24135289/python-opencv-detect-lines-in-a-tennis-court-using-two-differents-methods-of-h). This solution actually seems pretty close to the previous robust approach, except it's more lightweight. I'm probably going to try to implement Ben's suggestion simply because I want to try my hand at the RANSAC, which I've never used before. However, I *still* feel like the remarkable simplicity of the camerawork in tennis is not being fully leveraged here.

**Jeremie's Ricketty Approach**
...
**Jeremie's set of heuristics for court detection**
...
![Hough-line court detection example.](/figs/hough-line-ex-2.jpg){fig-alt="Hough-line court detection example." width="60%"}

## [2022-09-23] Tracking Games 02
...
![Hough-line court detection results, iteration 1.](/figs/hough-collage-1.jpg){fig-alt="Hough-line court detection results, iteration 1." width="90%"}
...
![Hough-line court detection results, iteration 2.](/figs/hough-collage-2.jpg){fig-alt="Hough-line court detection results, iteration 2." width="90%"}

## [2022-09-27] Court Detection 05
...
![Hough-line court detection results, iteration 3.](/figs/hough-collage-3.jpg){fig-alt="Hough-line court detection results, iteration 3." width="90%"}

## [2022-09-28] Change-up 06
...
![Hough-line court boundary detection.](/figs/hough-line-ex-4.jpg){fig-alt="Hough-line court boundary detection." width="90%"}
...
Update: While reading various strategies to detect players, I came across a [paper](https://www.researchgate.net/publication/221411677_Player_Detection_and_Tracking_in_Broadcast_Tennis_Video) outlining a strategy which uses non-dominant color extraction. I found an example of this on Stack and interpreted the code to get this:

![Non-dominant color extraction example for player detection.](/figs/hough-line-ex-3.jpg){fig-alt="Non-dominant color extraction example for player detection." width="90%"}

## [2022-09-29] Player Detection 01
Literature review today. Read a *fantastic* paper on table tennis tracking. Not only is the methodology explained clearly, the author provides details on the modules and parameters used in their research. Much appreciated. I'll try their method for ball detection. I can say some of their ideas will definitely make an impact on the court detection algorithm I've been writing.
...
![Player detection result.](/figs/player-detect-1.png){fig-alt="Player detection result." width="60%"}

## [2022-10-02] Player Detection 03
...
<video controls preload="metadata" width="100%" title="Player detection demonstration">
  <source src="/figs/player-detect-demo.mp4" type="video/mp4">
  Your browser does not support this video. <a href="/figs/player-detect-demo.mp4">Download the MP4 video.</a>
</video>

## [2022-10-12] General Update
